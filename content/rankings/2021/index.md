---
title: 'Ranking 2021'
date: 2025-12-01T15:02:28-06:00
images: ["/images/ranking/2021/listado-2021.png"]
draft: false
---

### Best Place To Code: Estas son las mejores empresas para trabajar en tecnología en 2021

<img src="/images/ranking/2021/listado-2021.png" class="img-fluid mx-auto d-block mb-4">

**México, Ciudad de México, 25 de enero de 2021.** - Software Guru, el medio y agencia de relacionamiento con desarrolladores de software líder en América Latina, dio a conocer el listado 2021 de las organizaciones donde es un placer trabajar como profesionista de tecnologías de información, independientemente del tamaño, ubicación o sector de la empresa.


Cientos de perfiles tecnológicos fueron quienes evaluaron de forma secreta y confidencial a la empresa en la que laboran. Como existen niveles mínimos de calificación necesarios para aprobar, no todas las empresas evaluadas lograron ser acreditadas como Best Place to Code®.


“Sabemos que 2020 fue un año muy difícil para muchas empresas debido a todos los cambios que se generaron por la pandemia. Sin embargo, varias empresas ya venían transformando su cultura y se adaptaron más rápido que otras. Un ejemplo claro es el trabajo remoto”, comentó Mara Ruvalcaba, directora de operaciones de director de Software Guru.

Por su parte, Pedro Galván, director de Software Guru, asegura que en estos últimos años han logrado que el mecanismo de evaluación sea el más transparente, confiable y práctico. “Muchas personas creen que el listado se conforma por empresas que pagan por aparecer pero la realidad es que no todas las empresas que se evalúan obtienen una calificación aprobatoria.”, afirmó.

Es bien sabido que ya existen otros programas o reconocimientos de “mejores empresas para trabajar”, pero Best Place to Code se distingue precisamente en este rubro por aspectos clave como evaluaciones confidenciales y realizadas por los mismos empleados. 

### 5 categorías para una comparación justa

Con base en las evaluaciones realizadas durante octubre-diciembre 2020, así ha quedado el ranqueo de empresas evaluadas como Best Place to Code.

El ranking está dividido en cinco categorías de acuerdo al tipo y tamaño de empresa.

### ISV o startup

Empresas que construyen sus propios productos digitales, ya sea para vender o como parte de su core de negocio.

1. Grow IT
2. Platzi
3. Creditas
4. Bitso
5. Contpaqi
6. OneCard
7. BITAM
8. Nu (¡Nueva!)
9. Obok (¡Nueva!)
10. Skydropx (¡Nueva!)

### Boutique

Empresas de consultoría tecnológica y agencias digitales con 10 o menos desarrolladores.

1. Roomie IT
2. OnePhase
3. Headspring Part of Accenture
4. Alluxi
5. Ordico

### Consultoría mediana

Empresas de consultoría tecnológica, desarrollo de software a la medida y team augmentation con entre 11 y 100 desarrolladores.

1. Aviada
2. Simpat Tech
3. Enroute Systems
4. Estrasol
5. michelada
6. Tango
7. AMK Technologies
8. Tacit Knowledge
9. Intelimétrica
10. DotNET
11. Scio Consulting
12. NT Group
13. Atalait
14. Montechelo
15. SERTI
16. Ensitech (¡Nueva!)
17. Enkoder (¡Nueva!)
18. Aptude (¡Nueva!)

### Consultoría global

Empresas de outsourcing tecnológico e integración de sistemas, con más de 100 desarrolladores.

1. Nova by Reboot
2. Grupo GFT
3. Svitla Systems
4. Luxoft
5. ITexico, an Improving company

### Corporativo

Empresas pertenecientes a sectores diversos que cuentan con una organización de tecnología interna para soportar su negocio.

1. BBVA
2. SkillForce MX
3. Edenred

¿Quieres conocer más a detalle a cada una? [Encuentra aquí el listado completo de Las Mejores Empresas para Trabajar en Tecnología 2020](https://bestplacetocode.com/).

### ¿Cómo desarrollan software estas empresas?

El próximo 10 y 11 de febrero del 2021 se realizará de manera online **Talks 2 Code**, una serie de pláticas técnicas impartidas por empresas acreditadas como Best Place to Code.

Durante estas charlas, diferentes empresas que obtuvieron la acreditación nos compartirán sus secretos y las mejores prácticas que utilizan para desarrollar software.

Así que no te pierdas las novedades de este evento por las redes sociales de Best Place to Code en [Facebook](https://www.facebook.com/bestplacetocode), [Twitter](https://twitter.com/bestplacetocode) e [Instagram](https://www.instagram.com/bestplacetocode/). También puedes unirte al [Slack](https://sg1.run/za) del evento para enterarte antes que nadie y estar en contacto con ponentes, empresas y colegas antes, durante y después de Talks 2 Code.


### Acerca de Best Place To Code

Es una iniciativa de Software Guru para reconocer a las empresas en las que recomendamos colaborar a quienes se desempeñan en las áreas de software y tecnología de información. Es decir, una organización donde es un placer trabajar como profesionista de tecnología de información, independientemente del tamaño, ubicación o sector de la empresa.
Antes que nada, "Best place to code" es un sello de calidad. Es una marca registrada por Software Guru, con la que acredita a las organizaciones que demuestran ser una opción atractiva para desarrolladores de software.


### Acerca de Software Guru

Software Guru es el medio y agencia de vinculación con desarrolladores de software líder en América Latina. A través de sus publicaciones, sitios web y eventos interactúa mensualmente con más de 80 mil desarrolladores de software de habla hispana. 